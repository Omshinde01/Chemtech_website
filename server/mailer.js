...
// Used by both the Express server (server/index.js) and the Vercel function (api/quote.js).
import nodemailer from "nodemailer";

const LIMITS = { name: 100, email: 254, company: 150, message: 5000 };
const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;

// Strip control chars (incl. CR/LF, which enable header injection) and trim.
const clean = (v, max) =>
  String(v ?? "")
...
export function validateQuote(body = {}) {
  ...
  if (data.message.length < 10) errors.message = "Please describe your requirement (at least 10 characters).";
...
export function buildEmail(data, meta = {}) {
  ...
  const subject = `New quote request from ${data.name}${data.company ? ` (${data.company})` : ""}`;
  ...
  const row = (label, value) =>
    `<tr><td style="padding:8px 16px 8px 0;color:#64748b;white-space:nowrap;vertical-align:top">${label}</td>` +
    `<td style="padding:8px 0;color:#0f172a">${value}</td></tr>`;
  ...
  <div style="font-family:Segoe UI,Arial,sans-serif;max-width:600px;margin:0 auto;color:#0f172a">
    <div style="background:#0B1C2C;color:#fff;padding:18px 24px;border-radius:10px 10px 0 0">
      <div style="font-size:12px;color:#60a5fa">Chemtech Specialty website</div>
      <div style="font-size:20px;font-weight:600;margin-top:4px">New quote request</div>
    </div>
    <div style="border:1px solid #e2e8f0;border-top:0;padding:20px 24px;border-radius:0 0 10px 10px">
      ...
      <div style="margin-top:16px;font-size:12px;color:#64748b">Requirements</div>
      <div style="margin-top:6px;padding:14px 16px;background:#f1f5f9;border-radius:8px;font-size:14px;line-height:1.6;white-space:pre-wrap">${escapeHtml(data.message)}</div>
      <p style="margin-top:18px;font-size:12px;color:#64748b">Hit Reply to respond directly to ${escapeHtml(data.name)}.</p>
    </div>
  </div>`;
...
function getTransport() {
  ...
  const { SMTP_HOST, SMTP_PORT = "465", SMTP_SECURE, SMTP_USER, SMTP_PASS } = process.env;
  if (!SMTP_HOST || !SMTP_USER || !SMTP_PASS) {
    throw new Error("Email is not configured: set SMTP_HOST, SMTP_USER and SMTP_PASS (see .env.example).");
  }
...
export async function sendQuoteEmail(data, meta = {}) {
  ...
  if (!to) throw new Error("Email is not configured: set MAIL_TO (see .env.example).");
  const { subject, text, html } = buildEmail(data, meta);
  const from = process.env.MAIL_FROM || process.env.SMTP_USER;
  await getTransport().sendMail({
    from: `"Chemtech Website" <${from}>`, // must be an address your SMTP account may send as
    to,
    replyTo: `"${data.name.replace(/"/g, "")}" <${data.email}>`, // Reply goes to the customer
    subject,
    text,
    html,
  });
...
// Framework-neutral request handler: returns { status, body }.
export async function handleQuote(body, meta = {}) {
  ...
  try {
    ...
    return { status: 502, body: { ok: false, error: "We couldn't send your inquiry right now. Please try again or email us directly." } };
  }
}
